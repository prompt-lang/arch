...
| **PNode** | 最小执行单元，支持 `llm`, `tool`, `function`, `planning`, `subworkflow`, `foreach`, `skill`, `mcp`, `openapi`, `grpc`, `script`, `memory`, `timer` 等多种类型 |
...
| **NNOS** | 运行时系统，支持调度、状态管理、重试、验证、资源配额、追踪、补偿、脚本执行、记忆后端等 |
...
所有字段名均为大小写敏感，推荐使用小写蛇形命名。

数据绑定表达式 {{...}} 中不允许包含副作用操作（如函数调用），仅支持路径访问和内置过滤器（| length, | join 等）。

工作流必须是有向无环图（DAG），除非 taskgraph 声明为 dynamic 或 hybrid 且由 planning 节点动态生成子图。

节点 id 在工作流内必须唯一，且符合标识符规范（字母、数字、下划线，不能以数字开头）。

所有 output.schema 遵循 JSON Schema 2020-12 草案，运行时使用该 schema 验证节点输出。
...
workflow:
  ...
  vars: 
    ...
    conversation_history: { type: array, ttl_sec: 1800 }  # v1.2 短期记忆过期
...
| `memory` | 记忆操作 | `operation`, `backend`, `collection` 等 |
  ...
  memory:
    ...
    backends:
      - id: "qdrant"
        ...
        default_collection: "pml_memories"
      - id: "local_kv"
        type: key_value
        path: "./memory.db"
```
...
pnodes:
  - id: retrieve
    type: memory
    operation: search          # read, write, search, update, delete, maintain
    backend: "qdrant"          # 可选，引用 config.memory.backends.id
    collection: "user_memories"
    vector: "{{embed.output.vector}}"
    limit: 5
    filter:
...
workflow:
  vars:
    conversation_history:
      type: array
      ttl_sec: 1800            # 30 分钟后自动清除
```

运行时在访问变量时惰性检查过期，或由后台维护任务清理。
...
- 记忆后端：向量数据库（Qdrant、Chroma）和键值存储（Redis、SQLite）
