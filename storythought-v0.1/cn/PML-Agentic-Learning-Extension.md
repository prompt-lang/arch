...
- **声明式学习**：使用 PML 节点直接描述训练循环、损失计算、参数更新，无需编写外部训练代码。
- **轻量快速训练**：支持小样本微调、在线学习、增量学习等场景，与 `control.loop` 和 `synthetic` 深度集成。
- **学习会话管理**：提供检查点、早停、指标记录能力，支持长期学习任务的可靠执行。
- **与现有生态无缝集成**：复用记忆（`memory`）、标记（`markup`）、多智能体（`agent`）、管线（`pipeline`）等现有组件。
